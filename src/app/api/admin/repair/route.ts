...
export async function POST(request: Request) {
  ...
  if (body.action === "v252_repair_preview") {
    const preview = await previewV252ProductionRepair({
      userId: owner.id,
      timezone: owner.timezone,
    });
    return NextResponse.json({ ok: true, preview });
  }
  if (body.action === "v252_repair_apply" && body.confirm === true) {
    const result = await applyV252ProductionRepair({
      userId: owner.id,
      timezone: owner.timezone,
    });
    return NextResponse.json({ ok: true, result });
  ...
  if (body.action === "v290_repair_preview") {
    return NextResponse.json({
      ok: true,
      preview: await previewV290ProductionRepair({ userId: owner.id }),
    });
  }
  if (body.action === "v290_repair_apply" && body.confirm === true) {
    return NextResponse.json({
      ok: true,
      result: await applyV290ProductionRepair({ userId: owner.id }),
    });
  }
  if (body.action === "v2100_repair_preview") {
    return NextResponse.json({
      ok: true,
      preview: await previewV2100ProductionRepair({ userId: owner.id }),
    });
  }
  if (body.action === "v2100_repair_apply" && body.confirm === true) {
    return NextResponse.json({
      ok: true,
      result: await applyV2100ProductionRepair({ userId: owner.id }),
    });
  }
  if (body.action === "v2110_repair_preview") {
    return NextResponse.json({
      ok: true,
      preview: await previewV2110ProductionRepair({ userId: owner.id }),
    });
  }
  if (body.action === "v2110_repair_apply" && body.confirm === true) {
    return NextResponse.json({
      ok: true,
      result: await applyV2110ProductionRepair({ userId: owner.id }),
    });
  }
  if (body.action === "v2120_repair_preview") {
    return NextResponse.json({
      ok: true,
      preview: await previewV2120ProductionRepair({ userId: owner.id }),
    });
  }
  if (body.action === "v2120_repair_apply" && body.confirm === true) {
    return NextResponse.json({
      ok: true,
      result: await applyV2120ProductionRepair({ userId: owner.id }),
    });
  }
  if (body.action === "v2130_repair_preview") {
    return NextResponse.json({
      ok: true,
      preview: await previewV2130ProductionRepair({ userId: owner.id }),
    });
  }
  if (body.action === "v2130_repair_apply" && body.confirm === true) {
    return NextResponse.json({
      ok: true,
      result: await applyV2130ProductionRepair({ userId: owner.id }),
    });
  }
  if (body.action === "v2140_repair_preview") {
    return NextResponse.json({
      ok: true,
      preview: await previewV2140ProductionRepair({ userId: owner.id }),
    });
  }
  if (body.action === "v2140_repair_apply" && body.confirm === true) {
    return NextResponse.json({
      ok: true,
  ...
  if (body.action === "v2170_repair_preview") {
    return NextResponse.json({
      ok: true,
      preview: await previewV2170ProductionRepair({ userId: owner.id }),
    });
  }
  if (body.action === "v2170_repair_apply" && body.confirm === true) {
    return NextResponse.json({
      ok: true,
      result: await applyV2170ProductionRepair({ userId: owner.id }),
    });
  }
  if (body.action === "v2180_repair_preview") {
    return NextResponse.json({
      ok: true,
      preview: await previewV2180ProductionRepair({ userId: owner.id }),
    });
  }
  if (body.action === "v2180_repair_apply" && body.confirm === true) {
    return NextResponse.json({
      ok: true,
  ...
  if (body.action === "v2200_repair_preview") {
    return NextResponse.json({
      ok: true,
      preview: await previewV2200ProductionRepair({
        userId: owner.id,
        timezone: owner.timezone,
      }),
    });
  }
  if (body.action === "v2200_repair_apply" && body.confirm === true) {
    return NextResponse.json({
      ok: true,
  ...
  if (body.action === "v2220_repair_preview") {
    return NextResponse.json({
      ok: true,
      preview: await previewV2220ProductionRepair({
        userId: owner.id,
        timezone: owner.timezone,
      }),
    });
  }
  if (body.action === "v2220_repair_apply" && body.confirm === true) {
    return NextResponse.json({
      ok: true,
      result: await applyV2220ProductionRepair({
        userId: owner.id,
        timezone: owner.timezone,
      }),
    });
  ...
  if (body.action === "v242_probe_cleanup" && body.confirm === true) {
    const result = await cleanupV242SnoozeProbe({ userId: owner.id });
    return NextResponse.json({ ok: true, result });
  }
